...
public class GuiIngameMixin {
    ...
    protected void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes)
    {
        ...
        int x1 = (int) (scaledRes.getScaledWidth() * (100f / 1920f));
        int y = (int) (scaledRes.getScaledHeight() - scaledRes.getScaledHeight() * (150f / 1080f));
        int y1 = (int) (scaledRes.getScaledHeight() * (150f / 1080f));
        int c = 0;
        ...
        for (ItemStack stack : stacks){
            ...
            int iX = (int) (x - (x1 + scaledRes.getScaledWidth() * (64f / 1920f)) / 2f);
            int iY = (int) (y - (y1 + scaledRes.getScaledWidth() * (64f / 1920f)) / 2f);
            if (JediMod.MOD.abilityCd.get(stack).getCd() == 0){
                if (a2){
                    drawRect(x - x1, y - y1, x, y, new Color(0.3f, 0.3f, 0.3f, 0.5f).getRGB());
                    GlStateManager.pushMatrix();
                    GlStateManager.translate((float) iX, (float) iY, 0.0f);
                    GlStateManager.scale(2, 2, 1);
                    GlStateManager.translate((float) -iX, (float) -iY, 0.0f);
                    Minecraft.getMinecraft().getRenderItem().renderItemIntoGUI(stack, iX, iY);
                    GlStateManager.popMatrix();
                    getFontRenderer().drawStringWithShadow("[" + s + "]", x - ((float) x1 / 2) - ((float) this.getFontRenderer().getStringWidth("[" + s + "]")) / 2, y - (scaledRes.getScaledHeight() * (18f / 1080f)), new Color(0.66f, 0.49f, 0f, 1f).getRGB());
                    c++;
                    if (c%5==0){
                        x = (int) (scaledRes.getScaledWidth() - scaledRes.getScaledWidth() * (20f / 1920f));
                        y-=(int) (scaledRes.getScaledHeight() * (200f / 1080f));
                    }else{
                        x-=(int) (scaledRes.getScaledWidth() * (150f / 1920f));
                    }
                }
            }else{
                ...
                if (JediMod.MOD.abilityCd.get(stack).getCd() < JediMod.MOD.abilityCd.get(stack).getMaxCd()){
                    drawRect(x - x1, y - y1, x, y - (int) (y1 * p), new Color(0.2f, 0.2f, 0.2f, 0.4f).getRGB());
                }
                GlStateManager.pushMatrix();
                GlStateManager.translate((float) iX, (float) iY, 0.0f);
                GlStateManager.scale(2, 2, 1);
                ...
                Minecraft.getMinecraft().getRenderItem().renderItemIntoGUI(stack, iX, iY);
                GlStateManager.popMatrix();
                drawRect(x - x1, y - (int) (y1 *  p), x, y, new Color(0.75f, 0f, 0f, 0.4f).getRGB());
                getFontRenderer().drawStringWithShadow("" + JediMod.MOD.abilityCd.get(stack).getCd(), x - (x1 / 2f) - ((float) this.getFontRenderer().getStringWidth("" + JediMod.MOD.abilityCd.get(stack).getCd())) / 2, y - (scaledRes.getScaledHeight() * (18f / 1080f)), new Color(0.6f, 0.6f, 0.6f, 1f).getRGB());
                c++;
                if (c%5==0){
                    x = (int) (scaledRes.getScaledWidth() - scaledRes.getScaledWidth() * (20f / 1920f));
                    y-=(int) (scaledRes.getScaledHeight() * (200f / 1080f));
    ...
    @Shadow @Final protected RenderItem itemRenderer;
